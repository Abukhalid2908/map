...
if (!in_array($mode,['create','reset'],true) || !filter_var($email,FILTER_VALIDATE_EMAIL) || strlen($email)>254 || strlen($password)<12 || strlen($password)>72) {
    fwrite(STDERR,"Set ADMIN_EMAIL and ADMIN_PASSWORD (12–72 bytes), then run admin-account.php create|reset.\n"); exit(1);
}
try {
    db()->beginTransaction();
    if ($mode==='create') query('INSERT INTO admins(email,password_hash) VALUES (?,?)',[$email,password_hash($password,PASSWORD_DEFAULT)]);
    else {
    ...
    db()->commit(); echo "Administrator account updated.\n";
